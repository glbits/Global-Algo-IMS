...
const TeamManagement = () => {
  ...
  const getChildRole = () => {
    if (myRole === 'Admin') {
      return formData.role === 'HR' ? 'HR' : 'BranchManager';
    }
    if (myRole === 'HR' || myRole === 'BranchManager') {
      return 'TeamLead';
    }
    if (myRole === 'TeamLead') {
      return 'Employee';
  ...
  const handleDelete = async (userId) => {
    if (!window.confirm('Are you sure? This will remove the user.')) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMsg({
        text: `Success! Created new ${childRole}: ${formData.name}`,
        type: 'success'
      });
      ...
      setMsg({
        text: err.response?.data?.msg || 'Failed to create user',
        type: 'error'
      });
    } finally {
      setLoading(false);
  ...
  // ================= ACCESS CONTROL =================
  if (myRole === 'Employee' || myRole === 'LeadManager') {
    return (
      <div className="p-10 text-center text-red-600 font-semibold">
        Access Denied: You are not allowed to create users.
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto flex gap-8">
      ...
      <div className="w-1/3 bg-white p-6 rounded-xl shadow-lg border-t-4 border-brand-medium h-fit">
        ...
        {msg.text && (
          <div
            className={`p-3 mb-4 rounded flex items-center gap-2 text-sm ${
              msg.type === 'success'
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* ADMIN ROLE SELECT (NO LEAD MANAGER) */}
          {myRole === 'Admin' && (
            <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-brand-medium text-white font-bold py-3 rounded-lg hover:bg-brand-dark transition"
          >
